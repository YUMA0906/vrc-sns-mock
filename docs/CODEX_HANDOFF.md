...
- 2026-07-08: Fixed hidden management buttons leaking into public profiles. Button-level `[hidden]` now wins over shared `.soft-button` / `.primary-button` display styles, so other people's profile pages only show membership detail/join actions and never the owner-only create/edit controls.
- 2026-07-08: Temporarily disabled profile support-history cards. `profileSupportCards` is now hidden and emptied from `renderProfileSubscriptionSection()`, while membership plan cards remain visible. Support-history badges in `profileSupportBadges` are now also limited to the owner profile so other people's profile pages do not show My Page-only support state.
- 2026-07-08: Made liked-post visibility an account-scoped privacy setting. `settingsLikedVisibility` now syncs to `profile.likedVisibility` per active account and uses keys like `settingsLikedVisibility:{activeAccountId}`. The owner can still view their own liked-post archive; public/profile display can hide the liked tab when the value is `private`.
...
- 2026-06-17: Improved post detail modal image fill. The foreground image still uses `contain` so the whole post image remains visible, while the image area now receives the same source image as a CSS variable and paints it behind the foreground with `cover`, blur, and subtle overlay layers. This fills empty side/top space for portrait and landscape images without cropping the actual image.
- 2026-06-17: Updated post detail modal image rendering so opened post images always use `object-fit: contain` and remain fully visible regardless of aspect ratio or source image size. The image entry animation now uses a slight vertical motion instead of scale-up to avoid transient cropping while the modal opens. Updated the stylesheet cache key in `index.html`.
- 2026-06-17: Ran a broad UI QA pass across home, post detail, public profile, my page, request manager, request detail, My Requests, events, event detail, settings, notifications, earnings, memberships, and tipping. Fixed remaining small/odd tap targets by expanding event carousel dot hit areas, making post-detail creator/category/tag/search links easier to tap, making the event organizer profile link a proper pill target, and forcing the post link-copy button to remain circular. Updated the stylesheet cache key in `index.html`.
- 2026-06-17: Added a mobile final-polish CSS layer for small screens. It tightens smartphone spacing, keeps the home tabs horizontally scrollable with the add button visible, reduces event banner height, constrains profile/request cards to the viewport, and improves request manager / My Requests card density without horizontal overflow.
...
- 2026-06-15: Restored the Featured Events carousel to the last stable autoplay behavior by removing the animation-state guard and `transitioncancel` handling. The carousel still uses cloned edge slides for natural loop wraparound, but it no longer blocks autoplay or drag if a transition event is missed.
- 2026-06-15: Upgraded the profile one-time tip card into a richer liquid-glass style card with stronger glass layers, soft highlights, and more visible amount chips.
- 2026-06-15: Added settings for one-time tipping. The Settings page now has a Tips panel with an account-scoped `Accept one-time tips` toggle, default ON, and a thank-you message textarea. If tips are disabled, public profiles hide the tip card; the owner's own profile always hides the tip card. The default thank-you message is `ありがとうございます！`.
- 2026-06-15: Added an X share action to the tip thank-you modal. After a tip is sent, the modal shows an `Xで報告` button that opens `https://twitter.com/intent/tweet` with auto text like `{creator}さんを支援しました！！` plus the creator profile URL.
